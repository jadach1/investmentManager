...
export class MiscLibrary{
    ...
    splitString(str: string) :string {
        ...
        let k: number = 0; //marker
        for(let i = 0; i < str.length; i++){
            if(str.charCodeAt(i) < 91) {
                newString += str.substring(k,i) + " ";
                k = i;
            }  
            if(i == str.length - 1){
                newString += str.substring(k,str.length)
            }
        }
...
export class ColourGenerator {
    ...
    fontList: string[] = [
      ...
      generateByLetter(symbol: string): String{
        let str: String;
        str = this.colorMap.get(symbol.substring(0,1));
        return str;
      }
  }